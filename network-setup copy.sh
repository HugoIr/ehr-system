...
# of configuration files for each organization in the "consortium/crypto-config/cryptogen"
...
# servers are in the "consortium/crypto-config/fabric-ca" directory. Within the same directory,
# the "registerEnroll.sh" script uses the Fabric CA client to create the identities,
# certificates, and MSP folders that are needed to create the test network in the
# "consortium/crypto-config/ordererOrganizations" directory.
...
function createOrgs() {
  ...
  ./consortium/crypto-config/ccp-generate.sh
}
